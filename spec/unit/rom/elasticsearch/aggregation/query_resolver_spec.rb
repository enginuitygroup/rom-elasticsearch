...
RSpec.describe ROM::Elasticsearch::Aggregation::QueryResolver do
  ...
  describe "with a single aggregation" do
    let(:aggregations) do
      [ROM::Elasticsearch::Aggregation.new(:sum).call("price")]
    end
  ...
  describe "with two aggregations" do
    let(:aggregations) do
      [
        ROM::Elasticsearch::Aggregation.new(:sum).call("price"),
        ROM::Elasticsearch::Aggregation.new(:avg).call("price")
      ]
    end
  ...
  describe "with a nested aggregation" do
    let(:parent_aggregation) do
      ROM::Elasticsearch::Aggregation.new(:range).tap do |agg|
        agg << child_aggregation
      end.call("price")
    end

    let(:child_aggregation) { ROM::Elasticsearch::Aggregation.new(:sum).call("price") }
